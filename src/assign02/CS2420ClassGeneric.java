...
public class CS2420ClassGeneric<Type> {
	...
	public boolean addStudent(CS2420StudentGeneric<Type> student) {
		for (CS2420StudentGeneric<Type> currentStudent : studentList) {
	...
	/**
	 *  Gets first student if the list isn't empty
	 * @return
	 */
	public CS2420StudentGeneric<Type> getFirstStudent() {
	...
	public CS2420StudentGeneric<Type> lookup(int uNID) {
		for (CS2420StudentGeneric<Type> currentStudent : studentList) {
			if (currentStudent.getUNID() == uNID) {
	...
	public ArrayList<CS2420StudentGeneric<Type>> lookup(Type contactInfo) {
		...
		for (CS2420StudentGeneric<Type> currentStudent : studentList) {
			if (currentStudent.getContactInfo().equals(contactInfo)) {
				contactList.add(currentStudent);
			}
		}
	...
	public double computeClassAverage() {
		...
		for (CS2420StudentGeneric<Type> currentStudent : studentList) {
			averageScore += currentStudent.computeFinalScore();
		}
		
		if (studentList.size() != 0) {
			averageScore /= studentList.size();
		}
		else {
			return 0;
		}
	...
	public ArrayList<Type> getContactList() {
		...
		for (CS2420StudentGeneric<Type> currentStudent : studentList) {
			if (!contactList.contains(currentStudent.getContactInfo())) {
				contactList.add(currentStudent.getContactInfo());
			}
		}
	...
	protected class OrderByName implements Comparator<CS2420StudentGeneric<Type>> {
		public int compare(CS2420StudentGeneric<Type> lhs, CS2420StudentGeneric<Type> rhs) {
			int comparison = lhs.getLastName().compareTo(rhs.getLastName());
			
			if (comparison == 0) {
				comparison = lhs.getFirstName().compareTo(rhs.getFirstName());
			}
			
			if (comparison == 0) {
				comparison = new OrderByUNID().compare(lhs, rhs);
			}
	...
	/**
	 * Comparator that defines ordering among students by score, breaking ties with uNID
	 * 
	 *
	 */
	protected class OrderByScore implements Comparator<CS2420StudentGeneric<Type>> {
		public int compare (CS2420StudentGeneric<Type> lhs, CS2420StudentGeneric<Type> rhs) {
			...
			else if (lhs.computeFinalScore() > rhs.computeFinalScore()) {
				comparison = -1;
			}
			else {
				comparison = new OrderByUNID().compare(lhs, rhs);
